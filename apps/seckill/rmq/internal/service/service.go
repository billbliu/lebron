...
import (
	...
	"github.com/billbliu/lebron/apps/seckill/rmq/internal/config"
...
func (s *Service) consume(ch chan *KafkaData) {
	...
	for {
		...
		_, err := s.ProductRPC.CheckAndUpdateStock(context.Background(), &productclient.CheckAndUpdateStockRequest{ProductId: m.Pid})
		if err != nil {
...
func (s *Service) consumeDTM(ch chan *KafkaData) {
	...
	for {
		...
		err := dtmgrpc.TccGlobalTransaction(dtmServer, gid, func(tcc *dtmgrpc.TccGrpc) error {
			if e := tcc.CallBranch(
				&productclient.UpdateProductStockRequest{ProductId: m.Pid, Num: 1},
